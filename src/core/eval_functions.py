...
def eval_intrinsic(
    ...
    device: str = "cuda",
    alpha: float = 0.5, 
    use_precalculated_embeddings: bool = False,
) -> float:
    ...
    with torch.no_grad():
        for student_batch, teacher_batch in zip(student_val_loader, teacher_val_loader):
            ...
            zero_vector = torch.zeros((1, teacher_batch.shape[1]), device=device)
            teacher_emb = torch.cat([zero_vector, teacher_batch.to(device)], dim=0)

            n = student_emb.shape[0]

            # Efficiently get all (i, j) pairs with i < j using torch.triu_indices
            idx = torch.triu_indices(n, n, offset=1, device=student_emb.device)
            idx_i, idx_j = idx[0], idx[1]

            # Vectorized computation for unique pairs (i < j)
            student_diff = student_emb[idx_i] - student_emb[idx_j]
            teacher_diff = teacher_emb[idx_i] - teacher_emb[idx_j]
            student_dist = torch.norm(student_diff, dim=1)
            teacher_dist = torch.norm(teacher_diff, dim=1)
            dist_loss = F.mse_loss(student_dist, teacher_dist)

            student_sim = F.cosine_similarity(student_emb[idx_i], student_emb[idx_j])
            teacher_sim = F.cosine_similarity(teacher_emb[idx_i], teacher_emb[idx_j])
            sim_loss = F.mse_loss(student_sim, teacher_sim)

            # Combined loss
            loss = alpha * sim_loss + (1 - alpha) * dist_loss

            batch_size = input_ids.size(0) if not use_precalculated_embeddings else student_batch.shape[0]
