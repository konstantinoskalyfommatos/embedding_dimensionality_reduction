...
# Idea
**Given an an embedding model that maps input text into vectors in $R^n$, learn a function that maps the output vectors in a lower dimensional space such that, given a batch of high-dimensional vectors, the low-dimensional vectors preserve the pairwise cosine similarities and euclidean distances.**
...
# Notes:
- If $f$ also preserves norms (i.e. $\|x\| = \|f(x)\| \forall x \in X$), then preserving the cosine of two vectors is equivalent to preserving their inner product.
...
- Define a simple Neural Network that maps output vectors into a low dimensinal space
...
$$\mathcal{L}_{\text{pos}} = \frac{1}{|B|(|B|-1)} \sum_{(i,j) \in \mathcal{P}} \left( \|x_i - x_j\|_2 - \|f_\theta(x_i) - f_\theta(x_j)\|_2 \right)^2$$
...
$$\mathcal{L}_{\text{sim}} = \frac{100}{|B|(|B|-1)} \sum_{(i,j) \in \mathcal{P}} \left( \cos(\theta_{ij}) - \cos(\phi_{ij}) \right)^2$$
...
**Remarks:**
- The number of pairs per batch is $|\mathcal{P}| = \binom{|B|}{2} = \frac{|B|(|B|-1)}{2}$
- When $\lambda = 1$, the network only preserves Euclidean distances
- When $\lambda = 0$, the network only preserves cosine similarities
